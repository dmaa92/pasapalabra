...
| 2026-09-04 | Most categories ship exactly two roscos, so two matches in the same category repeat the questions with the roles swapped. Fine for a one-off competition, thin for a long tournament. Eased on 2026-09-07 for Marvel Cinematic Universe (7), DC Comics and Deportes (6), Fórmula 1 and Fútbol Argentino (5); still true everywhere else. | Open |
| 2026-09-07 | Ten more category banks shipped on 2026-09-07 (Fórmula 1, Fútbol Argentino, Historia de Argentina, Geografía Argentina, Geografía de América, Fortnite, Minecraft, plus extra roscos for Deportes, Marvel Cinematic Universe and DC Comics). Like the 2026-09-04 batch they were written in-session by the model, not by `scripts/generate_rosco.py`, and every file still says `"reviewed_by_a_human": false`: the facts are unverified. | Open |
| 2026-09-07 | `scripts/check_banks.py` only compares the answers of the **first two** roscos of a file, so in a bank with more than two (Marvel, DC Comics, Deportes, Fórmula 1, Fútbol Argentino) an answer repeated between, say, the third and fourth rosco is not caught. The 2026-09-07 banks were written to be answer-unique across the whole file, but nothing enforces it. | Open |
